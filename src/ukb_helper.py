...
def run_ukb(input, output, algorithm):
    command = './ukb/src/ukb_wsd --{} -K ukb/scripts/wn30g.bin -D ukb/scripts/wn30_dict.txt ../data/clusters/{} > ../data/clusters/{}'.format(algorithm, input, output)
    subprocess.run(command, shell=True)
    
def prepare_data_ukb(file_name):
    
    cluster_bow = {}
    
    with open('../data/clusters/{}'.format(file_name), 'r') as f:
    ...
    # print(cluster_bow)
    with open('../data/clusters/temp_ukb_input.txt', 'w') as f:    
